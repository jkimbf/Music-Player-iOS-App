...
class PlayerViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        // 기준 시간 = seconds, 몇초로 나눌껀지 = preferredTimescale
        let timeInterval = CMTime(seconds: 1, preferredTimescale: 10)
        
        timeObserver = simplePlayer.addPeriodicTimeObserver(forInterval: timeInterval, queue: DispatchQueue.main, using: { time in
    ...
    @IBAction func seek(_ sender: UISlider) {
        ...
        let time = CMTime(seconds: seconds, preferredTimescale: 100) // 소수점 자릿 수 설정
